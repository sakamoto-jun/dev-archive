...
- Notion DB 연동 — `Published` 체크된 글만 자동 노출
...
### 2. 코드 블록 복사 시 `[object Object]` 출력

**문제**  
코드 블록의 내용을 복사할 때 React children을 그대로 문자열로 읽으려 하자 `[object Object]`가 복사되는 문제가 발생했다. children은 React 엘리먼트 객체이기 때문이다.

**해결**  
`useRef`로 실제 렌더링된 `<pre>` DOM 요소를 참조하고, `textContent`로 순수 텍스트만 추출했다.
